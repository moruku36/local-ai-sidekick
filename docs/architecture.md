...
Root AGENTS.md is the repository entrypoint for Lead clients that load it.
There is no verified Astra/Antigravity proprietary hook in this repository;
other clients must attach AGENTS.md and the policy as workspace instructions.
Worker system prompts remain unchanged and never invoke delegation.
