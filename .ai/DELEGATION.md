...
An optional `ai-dev-bootstrap` SessionStart integration (see
[README.md](../README.md#global-integration-optional)) may run once when the
Lead Host session starts. It only inspects local state (cwd, `.git`, `.ai/`,
lock files) and prints a short Development Context; it never runs tests,
Ollama inference, Factory verification, or any network/Git-remote operation,
and it never writes to the repository.
